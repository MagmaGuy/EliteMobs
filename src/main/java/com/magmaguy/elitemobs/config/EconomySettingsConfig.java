...
public class EconomySettingsConfig {
    ...
    public static void initializeConfig() {
        ...
        enableEconomy = ConfigurationEngine.setBoolean(List.of("Sets if the EliteMobs economy is enabled. This means elite coins, the ability to buy and sell gear and the ability to upgrade guild ranks", "If disabled, players will not be able to progress in the plugin!"), fileConfiguration, "enableEconomy", true);
        resaleValue = ConfigurationEngine.setDouble(List.of("Sets the resale item of items, as a % of the original price. 5 is 5%"), fileConfiguration, "itemResaleValue", 5);
        currencyName = ConfigurationEngine.setString(List.of("Sets the in-game name of the currency used."), file, fileConfiguration, "currencyName", "Elite Coins", true);
        useVault = ConfigurationEngine.setBoolean(List.of("Sets the plugin to use Vault. THIS IS NOT RECOMMENDED! Read why here: https://github.com/MagmaGuy/EliteMobs/wiki/%5BThird-party-support%5D-Vault"), fileConfiguration, "useVault - not recommended", false);
        enableCurrencyShower = ConfigurationEngine.setBoolean(List.of("Sets if elites will drop coins based on their level."), fileConfiguration, "enableCurrencyShower", true);
        currencyShowerMultiplier = ConfigurationEngine.setDouble(List.of("Sets the multiplier for the currency dropped by elites."), fileConfiguration, "currencyShowerTierMultiplier", 1D);
        chatCurrencyShowerMessage = ConfigurationEngine.setString(List.of("Sets the message sent when players pick up elite currency."), file, fileConfiguration, "chatCurrencyShowerMessage", "&7[EM] You've picked up &a$amount $currency_name!", true);
        actionBarCurrencyShowerMessage = ConfigurationEngine.setString(List.of("Sets the action bar message sent when players pick up elite currency."), file, fileConfiguration, "actionbarCurrencyShowerMessage", "&7[EM] You've picked up &a$amount $currency_name!", true);
        lootShowerMaterial1 = ConfigurationEngine.setString(List.of("Sets the material type of 1 dropped elite coin."), file, fileConfiguration, "lootShowerMaterial.1", Material.GOLD_NUGGET.name(), false);
        ConfigurationEngine.setInt(List.of("Sets the custom model ID for 1 dropped elite coin. Used by the resource pack."), fileConfiguration, "lootShowerData.1", 1);
        ...
        adventurersGuildNotificationMessage = ConfigurationEngine.setString(List.of("Send the message players get after looting currency. Useful for tutorial purposes."), file, fileConfiguration, "adventurersGuildNotificationMessages", "&7[EM] Extra spending money? Try &a/ag !", true);
        ...
        defaultMaterialWorth = ConfigurationEngine.setDouble(List.of("Sets the default material worth for items not specifically defined."), fileConfiguration, "materialWorth.defaultMaterialWorth", 1);
        ...
        economyPayMessage = ConfigurationEngine.setString(List.of("Message sent when sending elite currency to other players."), file, fileConfiguration, "Economy pay message v2", "&2You have paid &2$amount_sent $currency_name &2to $receiver&2, who got $amount_received after taxes!", true);
        economyCurrencyLeftMessage = ConfigurationEngine.setString(List.of("Message sent after players send currency."), file, fileConfiguration, "Economy currency left message", "You now have &2$amount_left $currency_name", true);
        economyPaymentReceivedMessage = ConfigurationEngine.setString(List.of("Message received when receiving currency."), file, fileConfiguration, "Economy money from payment message", "You have received &2$amount_received $currency_name &ffrom $sender", true);
        economyPaymentInsufficientCurrency = ConfigurationEngine.setString(List.of("Message sent when players try to send an amount of coins they do not have."), file, fileConfiguration, "Economy payment insufficient currency", "&cYou don't have enough $currency_name to do that!", true);
        economyWalletCommand = ConfigurationEngine.setString(List.of("/em balance message"), file, fileConfiguration, "Wallet command message", "You have &2$balance $currency_name", true);
        economyTaxMessage = ConfigurationEngine.setString(List.of("Confirmation message sent when players try to send currency to another player."), file, fileConfiguration, "Tax confirmation message", "&cSending a payment will cost $percentage% in taxes. &aDo &9$command &ato proceed!", true);
        shopBuyMessage = ConfigurationEngine.setString(List.of("Message sent when a player buys from a shop."), file, fileConfiguration, "Shop buy message", "&aYou have bought $item_name &afor $item_value $currency_name!", true);
        shopCurrentBalance = ConfigurationEngine.setString(List.of("Message sent when a player interacts with a shop."), file, fileConfiguration, "Shop current balance message", "&aYou have $currency_amount $currency_name.", true);
        shopInsufficientFundsMessage = ConfigurationEngine.setString(List.of("Message sent when players don't have enough currency to purchase an item"), file, fileConfiguration, "Shop insufficient funds message", "&cYou don't have enough $currency_name!", true);
        shopItemPrice = ConfigurationEngine.setString(List.of("Second part of message sent when players try to purchase an item they can not afford."), file, fileConfiguration, "Shop item cost message", "That item costs &c$item_value $currency_name.", true);
        shopSellMessage = ConfigurationEngine.setString(List.of("Message sent upon selling an item to a shop."), file, fileConfiguration, "Shop sell message", "&aYou have sold $item_name &afor $currency_amount $currency_name!", true);
        ...
        shopSaleInstructions = ConfigurationEngine.setString(List.of("Message sent upon trying to sell a non-EliteMobs item"), file, fileConfiguration, "Shop sale instructions", "&cYou can only sell EliteMobs loot here! (Armor / weapons dropped from elites showing a value on their lore)", true);
        shopBatchSellMessage = ConfigurationEngine.setString(List.of("Message sent upon selling a batch of elite items."), file, fileConfiguration, "shopBatchSellItem", "&aYou have sold your items &afor $currency_amount $currency_name!", true);

        enableMYSQL = ConfigurationEngine.setBoolean(List.of("Sets if the plugin should use MySQL for the economy system."), fileConfiguration, "useMySQL", false);
        MYSQLHost = ConfigurationEngine.setString(List.of("Sets the host of the MySQL server."), file, fileConfiguration, "MySQLHost", "localhost", true);
        MYSQLPort = ConfigurationEngine.setString(List.of("Sets the port of the MySQL server."), file, fileConfiguration, "MySQLPort", "3306", true);
        MYSQLDatabase = ConfigurationEngine.setString(List.of("Sets the database of the MySQL server."), file, fileConfiguration, "MySQLDatabase", "elitemobs_db", true);
        MYSQLUsername = ConfigurationEngine.setString(List.of("Sets the username of the MySQL server."), file, fileConfiguration, "MySQLUsername", " ", true);
        MYSQLPassword = ConfigurationEngine.setString(List.of("Sets the password of the MySQL server."), file, fileConfiguration, "MySQLPassword", " ", true);
        MYSQLTable = ConfigurationEngine.setString(List.of("Sets the table of the MySQL server."), file, fileConfiguration, "MySQLTable", "playerdata", true);
        MYSQLUseSSL = ConfigurationEngine.setBoolean(List.of("Sets if the MySQL server uses SSL."), fileConfiguration, "MySQLSSL", false);
