...
import com.magmaguy.elitemobs.advancedcombat.classes.BuiltInClassCatalog;
...
final class AdvancedClassCommandSupport {

    private AdvancedClassCommandSupport() {
    }

    static boolean requireModule(Player player) {
        if (AdvancedCombatModule.isInitialized()) return true;
    ...
    static void showForm(Player player, String formId) {
        ...
        ProfileSnapshot profile = AdvancedCombatModule.get().profile(player.getUniqueId()).orElse(null);
        if (profile == null) {
    ...
    static ClassCatalog catalog() {
        return AdvancedCombatModule.isInitialized()
                ? AdvancedCombatModule.get().catalog()
